...
Ряд Дирихле расходится при $0<p\leq1$.
...
{{< math >}}
Пусть $\sum\limits_{n=1}^{\infty} a_n$ и $\sum\limits_{n=1}^{\infty} b_n$ {{< /math >}} --- ряды с положительными членами, причём {{< math >}} $a_n \leq b_n$ для всех номеров $n$, начиная с некоторого. Тогда:

1) Если ряд $\sum\limits_{n=1}^{\infty} b_n$ сходится, то сходится и ряд $\sum\limits_{n=1}^{\infty} a_n$;

2) Если ряд $\sum\limits_{n=1}^{\infty} a_n$ расходится, то расходится и ряд $\sum\limits_{n=1}^{\infty} b_n$; {{< /math >}}
...
(В случае $l=1$ признак Даламбера не даёт возможности установить, сходится ряд или расходится.) {{< /math >}}
...
(В случае $l=1$ признак Коши не даёт возможности установить, сходится ряд или расходится.) {{< /math >}}
...
1) Абсолютные величины членов данного ряда положительны и монотонно убывают при $n \to \infty$, так как в числителе константа. 

\[ \lim\limits_{n \to \infty} a_n = \lim\limits_{n \to \infty} \dfrac{4}{(n+4)(n+23)} = 0  \]

2) Общий член ряда стремится к нулю при $n \to \infty$. {{< /math >}}
